...
if (php_sapi_name() !== 'cli') {
    ...
    $privateKey
        = 'aHR0cHM6Ly9kYXBpLm5ldC5wbC9wcm9qZWt0eS93eWtvcGFrYS9jaGVja1JlZ2lzdGVyZWREb21haW5zLnBocA';
    ...
    $currentState = !empty($fileContent) ? explode($divider, $fileContent)
        : [null, null];
    $currentState = count($currentState) < 2 ? [null, null] : $currentState;

    $resp = $currentState[1];

    if ($currentState[0] !== $serverName
        || (!empty($resp)
            && $resp !== 'true')
    ) {
